...
async function generateBracket(tournamentId, categoryId) {
    ...
    switch (format) {
        case 'single_elimination':
            matches = generateSingleEliminationBracket(tournamentId, categoryId, sortedRegistrations);
            break;
        case 'double_elimination':
            matches = generateDoubleEliminationBracket(tournamentId, categoryId, sortedRegistrations);
            break;
        case 'round_robin':
            matches = generateRoundRobinMatches(tournamentId, categoryId, sortedRegistrations);
            break;
        default:
 ...
 */
function generateSingleEliminationBracket(tournamentId, categoryId, registrations) {
    const matches = [];
    ...
    for (let i = 0; i < bracketSize / 2; i++) {
        ...
        const match = {
            ...
            participant1Id: p1 === null || p1 === void 0 ? void 0 : p1.id,
            participant2Id: p2 === null || p2 === void 0 ? void 0 : p2.id,
            status: 'scheduled',
            scores: [],
        };
 ...
 */
function generateDoubleEliminationBracket(tournamentId, categoryId, registrations) {
    const numParticipants = registrations.length;
    const bracketSize = Math.pow(2, Math.ceil(Math.log2(numParticipants)));
    ...
    for (let round = 1; round <= winnersRounds; round++) {
        ...
        for (let i = 0; i < matchesInRound; i++) {
            ...
            if (round === 1) {
                const p1 = positions[i * 2];
                const p2 = positions[i * 2 + 1];
                match.participant1Id = p1 === null || p1 === void 0 ? void 0 : p1.id;
                match.participant2Id = p2 === null || p2 === void 0 ? void 0 : p2.id;
                // Handle byes
                if (p1 && !p2) {
                    match.status = 'completed';
                    match.winnerId = p1.id;
    ...
    for (let round = 0; round < winnersMatchesByRound.length; round++) {
        ...
        for (let i = 0; i < currentRoundMatches.length; i++) {
            ...
            if (losersMatchesByRound.length > 0) {
                ...
                // WR1 losers -> LR1, WR2 losers -> LR2, etc.
                const losersRoundForDropdown = round < losersMatchesByRound.length ? round : losersMatchesByRound.length - 1;
 ...
 */
function generateRoundRobinMatches(tournamentId, categoryId, registrations) {
    const matches = [];
 ...
 */
function processAdvanceByeWinners(matches) {
    var _a;
    ...
    while (advancementMade) {
        ...
        for (let i = 0; i < matches.length; i++) {
            ...
            if ((_a = match.nextMatchId) === null || _a === void 0 ? void 0 : _a.startsWith('MATCH_')) {
 ...
 */
function calculateByePositions(bracketSize, numByes) {
    const positions = [];
    if (numByes === 0)
        return positions;
 ...
 */
async function updateNextMatchReferences(tournamentId, categoryId, matches, matchRefs) {
    var _a, _b;
    const db = getDb();
    const batch = db.batch();
    for (let i = 0; i < matches.length; i++) {
        ...
        if ((_b = match.loserNextMatchId) === null || _b === void 0 ? void 0 : _b.startsWith('MATCH_')) {
