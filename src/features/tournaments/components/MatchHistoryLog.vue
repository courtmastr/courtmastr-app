<template>
  <v-card>
    ...
    <v-card-text>
      <v-timeline v-if="events.length > 0" side="end" density="compact">
        <v-timeline-item
          ...
          <v-card variant="tonal">
            <v-card-text class="py-2">
              <div class="d-flex align-center">
                <v-icon :color="getEventColor(event.type)" size="small" class="mr-2">
                  {{ getEventIcon(event.type) }}
                </v-icon>
                <span class="font-weight-medium">{{ event.title }}</span>
              </div>
              <div v-if="event.description" class="text-caption text-grey mt-1">
                {{ event.description }}
              </div>
            </v-card-text>
          </v-card>
